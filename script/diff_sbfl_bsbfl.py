...
for i in lst:
    if(i[1]==i[3]):
        continue
    result.append(float(i[1])/float(i[5])*100.0)
...
if False:
    print(str(round(float(args[2]),2))+'&'+str(round(sbfl_ave/(len(result)/2)*100,2))+'&'+str(round(bsbfl_ave/(len(result)/2)*100,2))+'\\\\')
...
if False:
    ...
    fig.savefig("./../figure/bar/diff_sbfl_bsbfl_"+args[1]+args[2]+".pdf",bbox_inches="tight",pad_inches=0.05)